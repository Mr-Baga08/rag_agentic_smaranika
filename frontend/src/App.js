...
export default function TheTruthSchoolAI() {
  ...
  // Agentic RAG state
  const [agentMode, setAgentMode] = useState(() => getFromLocalStorage('agentMode', false));
  const [showToolSteps, setShowToolSteps] = useState(() => getFromLocalStorage('showToolSteps', true));
  ...
  useEffect(() => {
    try {
      ...
      window.localStorage.setItem('agentMode', JSON.stringify(agentMode));
      window.localStorage.setItem('showToolSteps', JSON.stringify(showToolSteps));
    } catch (error) {
      console.error('Error saving to localStorage:', error);
    }
  }, [darkMode, messages, selectedDomain, processingDocs, enableWebSearch, webSearchOnly, fastMode, enableCache, enableQueryImprovement, enableVerification, agentMode, showToolSteps]);
  ...
  const fetchAgentTools = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/agent/tools?domain=${selectedDomain}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleAgentQuery = async () => {
    ...
    setMessages(prev => [
      ...prev,
      { role: 'assistant', content: '', streaming: true, agentMode: true, toolCalls: [], toolsUsed: [] }
    ]);
    ...
    try {
      ...
      if (data.success) {
        setMessages(prev => {
          ...
          newMessages[assistantMessageIndex] = {
            ...
            streaming: false,
            agentMode: true,
            toolCalls: data.intermediate_steps || [],
            toolsUsed: data.tools_used || [],
            numSteps: data.num_steps || 0
          };
          return newMessages;
        });
      } else {
      ...
      setError(`Agent query failed: ${err.message}`);
  ...
  const getToolIcon = (toolName) => {
    const icons = {
      ...
      mathematical_analysis: <Brain className="w-4 h-4" />,
      data_analysis: <BarChart3 className="w-4 h-4" />,
      visualization: <Image className="w-4 h-4" />,
      rag_query: <FileText className="w-4 h-4" />
    };
  ...
  const getToolColor = (toolName) => {
    const colors = {
      code_executor: 'text-green-500',
      mathematical_analysis: 'text-purple-500',
      data_analysis: 'text-blue-500',
      visualization: 'text-pink-500',
      rag_query: 'text-orange-500'
    };
    return colors[toolName] || 'text-gray-500';
  };

  const toggleStepExpansion = (msgIdx, stepIdx) => {
    setExpandedSteps(prev => ({
      ...prev,
      [`${msgIdx}-${stepIdx}`]: !prev[`${msgIdx}-${stepIdx}`]
    }));
  };
              ...
              >
                <div className="max-w-3xl mx-auto flex gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {msg.agentMode && msg.toolCalls && msg.toolCalls.length > 0 && showToolSteps && (
                      <div className="mt-4 space-y-2">
                        <div className={`flex items-center gap-2 ${theme.textMuted} text-xs font-semibold uppercase tracking-wide mb-3`}>
                          <Zap className="w-3.5 h-3.5" />
                          <span>Tools Used ({msg.toolsUsed?.length || 0})</span>
                        </div>
                        ...
                        {msg.toolsUsed && msg.toolsUsed.length > 0 && (
                          <div className="flex flex-wrap gap-2 mb-3">
                            {msg.toolsUsed.map((tool, i) => (
                              <div key={i} className={`flex items-center gap-1.5 px-3 py-1.5 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-100 border-gray-200'} border rounded-full text-xs`}>
                                <span className={getToolColor(tool)}>
                                  {getToolIcon(tool)}
                                </span>
                        ...
                        {/* Detailed steps */}
                        <div className="space-y-2">
                          {msg.toolCalls.map((step, stepIdx) => {
                            const isExpanded = expandedSteps[`${idx}-${stepIdx}`];
                            return (
                              <div key={stepIdx} className={`${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-50 border-gray-200'} border rounded-lg overflow-hidden`}>
                                <button
                                  onClick={() => toggleStepExpansion(idx, stepIdx)}
                                  className={`w-full px-4 py-3 flex items-center justify-between ${theme.hover} transition-colors`}
                                >
                                  <div className="flex items-center gap-3">
                                    <div className={`flex items-center justify-center w-6 h-6 rounded-full ${darkMode ? 'bg-gray-700' : 'bg-gray-200'} ${theme.textMuted} text-xs font-semibold`}>
                                      {stepIdx + 1}
                                    </div>
                                    <span className={getToolColor(step.tool)}>
                                ...
                                {isExpanded && (
                                  <div className={`px-4 pb-4 space-y-3 ${theme.textSecondary} text-sm`}>
                                    <div>
                                      <div className={`${theme.textMuted} text-xs uppercase tracking-wide font-semibold mb-1.5`}>Input</div>
                                      <div className={`${darkMode ? 'bg-gray-900/50' : 'bg-white'} p-3 rounded border ${theme.borderLight} font-mono text-xs overflow-x-auto`}>
                                        {typeof step.tool_input === 'string' ? step.tool_input : JSON.stringify(step.tool_input, null, 2)}
                                      </div>
                                    </div>
                                    <div>
                                      <div className={`${theme.textMuted} text-xs uppercase tracking-wide font-semibold mb-1.5`}>Output</div>
                                      <div className={`${darkMode ? 'bg-gray-900/50' : 'bg-white'} p-3 rounded border ${theme.borderLight} text-xs overflow-x-auto whitespace-pre-wrap`}>
                                        {step.observation}
                                      </div>
                                    </div>
                                  </div>
                        ...
                        {/* Summary */}
                        <div className={`text-xs ${theme.textMuted} mt-3 flex items-center gap-2`}>
                          <CheckCircle className="w-3.5 h-3.5 text-green-500" />
                          <span>Completed in {msg.numSteps || msg.toolCalls.length} step{msg.numSteps !== 1 ? 's' : ''}</span>
                        </div>
                      </div>
                    )}
            ...
            <label className="flex items-center space-x-2 cursor-pointer">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.checked) {
                    // Disable web search when agent mode is on
              ...
              <div className="flex items-center space-x-1.5">
                <Zap className={`w-3.5 h-3.5 ${agentMode ? 'text-purple-500' : theme.textSecondary}`} />
                <span className={`text-sm ${agentMode ? 'text-purple-500 font-medium' : theme.textSecondary}`}>
                  Agent Mode {agentMode && '(Tools: Code, Math, Data, Viz)'}
                </span>
              </div>
            </label>
        ...
        <div className="space-y-6">
          ...
          <div className={`${theme.bgTertiary} ${theme.border} border rounded-lg p-6`}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className={`text-lg font-semibold ${theme.text}`}>Agentic RAG Settings</h3>
            </div>
            <div className="space-y-4">
              <div className="flex items-start space-x-3">
                <input
                  type="checkbox"
                  id="agentModeSettings"
                  checked={agentMode}
                  onChange={(e) => setAgentMode(e.target.checked)}
                  className="w-5 h-5 text-purple-600 rounded focus:ring-purple-500 mt-0.5"
                />
                <div className="flex-1">
                  <label htmlFor="agentModeSettings" className={`block text-sm font-medium ${theme.text} cursor-pointer`}>
                    Enable Agent Mode
                  </label>
                  <p className={`text-xs ${theme.textSecondary} mt-1`}>
                    Use intelligent agents with code execution, math, data analysis, and visualization tools
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                />
                <div className="flex-1">
                  <label htmlFor="showToolSteps" className={`block text-sm font-medium ${agentMode ? theme.text : theme.textMuted} cursor-pointer`}>
                    Show Tool Execution Steps
                  </label>
                  <p className={`text-xs ${theme.textSecondary} mt-1`}>
                    Display detailed steps of tool calls and intermediate results
                  </p>
                </div>
              </div>

              {agentMode && availableTools.length > 0 && (
                <div className={`mt-4 p-4 ${darkMode ? 'bg-gray-800/30' : 'bg-gray-50'} rounded-lg`}>
                  <div className={`text-xs font-semibold ${theme.textMuted} uppercase tracking-wide mb-3`}>
                    Available Tools ({availableTools.length})
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {availableTools.map((tool, i) => (
                      <div key={i} className={`flex items-center gap-2 p-2 ${darkMode ? 'bg-gray-800' : 'bg-white'} rounded border ${theme.border}`}>
