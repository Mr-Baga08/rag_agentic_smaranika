...
import sys
# Add repository root for .vendor/lightrag
sys.path.insert(0, str(Path(__file__).parent.parent.parent))
# Add project root for raganything and backend imports
...
from backend.agent_executor import create_agentic_rag_executor, AgenticRAGExecutor
...
# Agentic RAG executors per domain
agent_executors: Dict[str, AgenticRAGExecutor] = {}
...
class AgentQueryRequest(BaseModel):
    ...
    return_steps: bool = Field(True, description="Include intermediate tool call steps")

    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "query": "Calculate the derivative of x^2 + 3x + 2 and plot it",
                "domain": "general",
...
class AgentQueryResponse(BaseModel):
    ...
    intermediate_steps: List[Dict[str, Any]] = Field(default_factory=list, description="Tool calls made")
    tools_used: List[str] = Field(default_factory=list, description="List of tools used")
    num_steps: int = Field(0, description="Number of steps taken")
    conversation_id: str = Field(..., description="Conversation ID")
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "success": True,
                "output": "The derivative is 2x + 3. Here's the plot...",
                "intermediate_steps": [
                    {"tool": "mathematical_analysis", "tool_input": "differentiate x^2 + 3x + 2", "observation": "Result: 2x + 3"}
                ],
...
def get_or_create_agent_executor(domain: str) -> AgenticRAGExecutor:
    """Get existing agent executor or create a new one for the domain."""
    if domain not in agent_executors:
        logger.info(f"Creating new agent executor for domain: {domain}")

        # Get RAG instance if available
        rag_instance = rag_instances.get(domain)
        ...
        logger.info(f"✓ Agent executor created for domain: {domain}")

    return agent_executors[domain]


@app.post("/agent/query", response_model=AgentQueryResponse)
async def agent_query(request: AgentQueryRequest):
    """
    Execute a query using the agentic RAG system.

    The agent can use multiple tools:
    - Code execution (Python)
    - Mathematical analysis (SymPy)
    - Data analysis (Pandas)
    - Visualization (Matplotlib)
    - RAG query (document search)
    """
    try:
        logger.info(f"Agent query: '{request.query}' (domain: {request.domain})")

        # Validate domain
        if request.domain not in DOMAIN_CONFIGS and request.domain != "general":
            raise HTTPException(
                status_code=400,
                detail=f"Invalid domain: {request.domain}. Available: {list(DOMAIN_CONFIGS.keys()) + ['general']}"
            )

        # Get or create agent executor
        agent = get_or_create_agent_executor(request.domain)

        # Update RAG instance if available
        if request.domain in rag_instances:
            agent.update_rag_instance(
                rag_instance=rag_instances[request.domain],
                domain=request.domain
            )

        # Generate conversation ID if not provided
        conversation_id = request.conversation_id or f"agent_conv_{uuid.uuid4().hex[:12]}"
        ...
        response = AgentQueryResponse(
            ...
            intermediate_steps=result.get("intermediate_steps", []) if request.return_steps else [],
        ...
        logger.info(f"Agent query completed: {len(result.get('tools_used', []))} tools used, {result.get('num_steps', 0)} steps")
        return response
...
async def agent_query_stream(request: AgentQueryRequest):
    """
    Execute a query using the agentic RAG system with streaming responses.

    Returns Server-Sent Events (SSE) with real-time updates.
    """
    try:
        logger.info(f"Agent stream query: '{request.query}' (domain: {request.domain})")
        ...
        # Get or create streaming agent executor
        if request.domain not in agent_executors:
            agent_executors[request.domain] = create_agentic_rag_executor(
                gemini_api_key=GEMINI_API_KEY,
                rag_instance=rag_instances.get(request.domain),
                domain=request.domain,
                streaming=True,
                model_name=GEMINI_TEXT_MODEL,
                temperature=0.7,
        ...
        agent = agent_executors[request.domain]
        ...
        logger.error(f"Agent stream query error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Agent stream query failed: {str(e)}")


@app.get("/agent/tools")
async def get_agent_tools(domain: str = "general"):
    """Get list of available tools for the agent."""
    try:
        agent = get_or_create_agent_executor(domain)
...
@app.delete("/agent/conversation/{conversation_id}")
async def clear_agent_conversation(conversation_id: str, domain: str = "general"):
    """Clear agent conversation history."""
    try:
        if domain in agent_executors:
            agent = agent_executors[domain]
            agent.clear_history()
            logger.info(f"Cleared agent conversation: {conversation_id} (domain: {domain})")
            return {"success": True, "message": "Agent conversation cleared"}

        return {"success": True, "message": "No conversation found"}

    except Exception as e:
        logger.error(f"Error clearing agent conversation: {e}", exc_info=True)
